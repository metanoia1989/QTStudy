...
{
    ui->setupUi(this);
    QStringList drivers = QSqlDatabase::drivers();	// 查找数据库驱动并保存
    ui->comboDriver->addItems(drivers);	// 将这些驱动名加入组合框
...
{
    if (text == "QSQLITE") {	// SQLite 数据库是程序本地数据库，无需数据库名、用户名等参数
...
void ConnDlg::on_okButton_clicked()
{
    if (ui->comboDriver->currentText().isEmpty()) {
        ...
        if (err.type() != QSqlError::NoError) {
            ...
            ui->status_label->setText(tr("数据库连接成功！"));
        }
        // 创建数据表，如已存在则无需执行
        accept();
...
void ConnDlg::addSqliteConnection()
{
    ...
    ui->status_label->setText(tr("创建 slqite 数据库成功！"));
}
...
void ConnDlg::createDb()
{
    ...
    query.exec("create table cars (carid int primary key, name varchar(50), "
        "factoryid int, year int, foreign key(factoryid) references factory )");

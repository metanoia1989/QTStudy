...
void MainWindow::openTable()
{
    ...
    // theSelection 当前项变化时触发currentChanged信号
    ...
    ui->tableView->setModel(tabModel); // 设置模型数据
    ...
    // tableView 上为性别和部门两个字段设置自定义代理组件
    QStringList strList;
    strList << "男" << "女";
    bool isEditable = false;
    delegateSex.setItems(strList, isEditable);
    ui->tableView->setItemDelegateForColumn(
        tabModel->fieldIndex("Gender"), &delegateSex); // ComboBox选择型
    ...
    ui->tableView->setItemDelegateForColumn(
        tabModel->fieldIndex("Department"), &delegateDepart); // ComboBox选择型
...
//    dataMapper->addMapping(ui->dbLabPhoto, tabModel->fieldIndex("Photo")); // 图片无法直接映射

    dataMapper->toFirst(); // 移动到首记录
    getFieldNames(); // 获取字段名称列表，填充ui->groupBoxSort组件

    // 更新actions 和 界面组件的使能状态
...
void MainWindow::on_actSubmit_triggered()
{
    ...
    if (!res) {
        QMessageBox::information(this, "消息", "消息保存错误，错误信息\n"+tabModel->lastError().text(),
                                 QMessageBox::Ok, QMessageBox::NoButton);
    } else {
...
void MainWindow::on_actPhoto_triggered()
{
    ...
    QByteArray data;
    QFile *file = new QFile(aFile); // fileName 为二进制数据文件名

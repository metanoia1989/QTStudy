...
class CallMatrixWinUi(QWidget):
    ...
    def uiAccept(self):
        ...
        print("The volume of drinks is {0} liters {1} jiggers".format(
            self.getLiters(), self.getJiggers()))
        print("The blender is running at speed '{0}'".format(
            self.getSpeedName()))
        msg1 = "饮料量为：{0}升（{1} 个量酒器）。".format(
            self.getLiters(), self.getJiggers())

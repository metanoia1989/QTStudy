...
Textbook::Textbook(QString title, QString author, QString isbn,
    uint year, QObject *parent) : QObject(parent)
    , m_Title(title), m_Author(author)
    , m_Isbn(isbn), m_YearPub(year)
{
...
QString Textbook::toString() const
{
    return QString("[%1]:Title: %2; Author: %3; ISBN: %1; Year: %4")
        .arg(m_Isbn).arg(m_Title).arg(m_Author).arg(m_YearPub);
}

...
void Sqlite::openDB(QString db_name)
{
    ...
    db.setHostName("database-localhost");          //设置数据库主机名
    db.setDatabaseName(db_name);              //设置数据库名
    db.setUserName("educationsystem");                //设置数据库用户名
 ...
 * @return true/false
 */
bool Sqlite::hasTable(QString tb_name)
{
    return db.tables().contains(tb_name);
}

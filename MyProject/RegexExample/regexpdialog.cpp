    ...
    : QDialog(parent)
{
    ...
    palette.setBrush(QPalette::Base, palette.brush(
    ...
    syntaxComboBox->addItem(tr("Wildcard"), QRegExp::RegExp2);
    ...
    connect(patternComboBox, &QComboBox::editTextChanged, this,
            &RegExpDialog::refresh);
    connect(textComboBox, &QComboBox::editTextChanged, this,
            &RegExpDialog::refresh);
    connect(caseSensitiveCheckBox, &QAbstractButton::toggled, this,
            &RegExpDialog::refresh);
    connect(minimalCheckBox, &QAbstractButton::toggled, this,
            &RegExpDialog::refresh);
    connect(syntaxComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &RegExpDialog::refresh);

    patternComboBox->addItem(tr("[A-Za-z]+([A-Za-z0-9]*)"));
...
void RegExpDialog::refresh()
{
    ...
    for (int i=0; i<MaxCaptures; ++i) {
        captureLabels[i]->setEnabled(i <= rx.matchedLength());
